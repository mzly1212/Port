        ...
        for rv in raw_vehicles:
            ...
            if old_veh:
                ...
                if '137438953506' in self.map_mgr.lanes:
                    ...
                    if dist_506 < 2.5 and old_veh.haiyi_fitted_heading is not None:
                        ...
                        veh_heading = (90 - veh_heading) % 360      # 转换为前端匹配用的地理航向 (0北 90东)
            ...
            if fixed_id not in self.active_vehicles:
                ...
                if veh.lane_id == lane_id:
                    ...
                    if veh.lane_change_counter >= 5 and abs(veh.filtered_l) > 3.0:  # 1.2
                        # ✅ 正式确认变道！
                        veh.s_history.clear()
                        veh.lane_id = lane_id
                    ...
                    if new_x is not None:
                        ...
                        # 当前雷达真实航向(地理角)与目标地理航向的差值
                        veh.lc_offset_heading = (veh.raw_heading - target_geo + 180) % 360 - 180
                    # ==========================================
                    ...
                    # 初始化平滑滤波起点，防止首次输出为空报错
                    if veh.out_x is None:
                        veh.out_x = veh.raw_x
                        veh.out_y = veh.raw_y
                        ...
                        if abs(gap) > 30.0:
                            ...
                            if gap > 0:
                                ...
                                if veh.s <= veh.target_s:
                                    ...
                                    veh.is_chasing = False
                        # ❌❌❌ 核心修改：将下面的这三行代码完全删除！ ❌❌❌
                        # 绝对不能用推演车(视觉假象)的坐标去覆盖 raw_x/raw_y！
                        # 这样下一帧防劫持护盾依然会对比“上一帧的真实雷达点”和“这一帧的真实雷达点”，彻底消灭克隆车！
    ...
    def _generate_processed_vehicles(self, current_time):
        ...
        for veh in self.active_vehicles.values():
            ...
            if veh.is_off_lane:
                ...
                if not veh.is_off_lane:
                    # 在轨车辆应用输出级平滑滤波
                    x, y = self._alpha_filter_xy(veh, x, y, is_lc=getattr(veh, 'is_changing_lane', False))
