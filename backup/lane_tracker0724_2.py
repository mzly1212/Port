...
class LaneQueueTracker:
    ...
    def process_frame(self, raw_vehicles, current_time):
        ...
        for rv in raw_vehicles:
            ...
            # ==========================================
            # 🚀 升级：海一路西侧入轨轨迹拟合与向量夹角车道仲裁
            # 参考 linear_fitting.py 几何逻辑
            # ==========================================
            ...
            if old_veh:
                ...
                if '137438953506' in self.map_mgr.lanes and '137438953490' in self.map_mgr.lanes:
                    ...
                    if dist_506 < 2.5 and getattr(old_veh, 'haiyi_fitted_math_angle', None) is not None:
                        ...
                        angle_506 = self.map_mgr.lanes['137438953506']['heading']
                        angle_490 = self.map_mgr.lanes['137438953490']['heading']
                        ...
                        if diff_506 <= diff_490:
                            forced_haiyi_lane = '137438953506'
                        else:
                            forced_haiyi_lane = '137438953490'

                        # 将雷达瞬时航向也劫持为拟合航向，提高稳定性
                        veh_heading = old_veh.haiyi_fitted_heading

                        print(f"===== [海一路向量仲裁] ID:{int(fixed_id)%10000} | 轨迹角:{fit_angle:.1f}° | 对506夹角:{diff_506:.1f}° | 对490夹角:{diff_490:.1f}° | 最终裁决: {forced_haiyi_lane} =====")

                        if not old_veh.is_off_lane:
                            old_veh.haiyi_fitted_math_angle = None

            # ==========================================
            # 🚀 改进：坐标 (x,y) 与 航向角 (heading) 双层前置预滤波
            # ==========================================
            if old_veh:
                ...
                match_heading = rv.radar_heading
